...
import { ARTIST_SCENES, BACKDROP_COLORS, type ArtistScene } from "@/lib/live-studio-data";
...
export function ArtistShootStudio({ session }: Props) {
  const [sceneId, setSceneId] = useState(ARTIST_SCENES[0].id);
  const [backdropColorId, setBackdropColorId] = useState("cobalt");
  ...
  const scene: ArtistScene = ARTIST_SCENES.find(s => s.id === sceneId) ?? ARTIST_SCENES[0];
  const backdropColor = BACKDROP_COLORS.find(c => c.id === backdropColorId) ?? BACKDROP_COLORS[1];
  ...
  async function uploadRef(file: File): Promise<string> {
    ...
    const path = `${session.user.id}/artist-refs/${Date.now()}.${ext}`;
  ...
  async function generate() {
    ...
    try {
      ...
      const prompt = scene.hasColorPicker
        ? scene.prompt(backdropColor.promptName)
        : scene.prompt();
      const res = await fetch(`${AURORA_URL}/api/public/generate`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` },
        body: JSON.stringify({ kind: "image", prompt, imageUrls: [imageUrl], model: MODEL }),
      });
      const data: unknown = await res.json();
      if (!res.ok) throw new Error((data as { error?: string })?.error ?? "Generation failed");
      toast.success("Artist shot queued! Rendering now…");
      await fetchGallery();
    } catch (err) {
  ...
  const t = { fontFamily: "'Inter', sans-serif", color: "var(--text)" };

  return (
    <div style={{ ...t, minHeight: "100vh", background: "var(--bg)" }}>
      {/* Header */}
      <header style={{ borderBottom: "1px solid var(--border)", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", background: "var(--bg-card)", position: "sticky", top: 0, zIndex: 50, backdropFilter: "blur(12px)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg, #ec4899, #f97316)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontSize: 18 }}>📸</span>
          </div>
          <div>
            <div style={{ fontWeight: 700, fontSize: 16 }}>Artist Shoot Studio</div>
            <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Music video sets, colored backdrops, arena stage</div>
          </div>
        </div>
        <button onClick={() => supabase.auth.signOut()} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: 8, border: "1px solid var(--border)", background: "transparent", color: "var(--text-muted)", cursor: "pointer", fontSize: 13 }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 20px", display: "grid", gridTemplateColumns: "1fr 340px", gap: 28 }}>
        {/* Left: Scene picker */}
        <div>
          <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 14 }}>Pick a scene</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12, marginBottom: 20 }}>
            {ARTIST_SCENES.map(sc => (
              <button
                key={sc.id}
                ...
                style={{
                  ...
                  aspectRatio: "16/9",
                  borderRadius: 12,
                  border: `2px solid ${sceneId === sc.id ? "#ec4899" : "var(--border)"}`,
                  overflow: "hidden",
                  cursor: "pointer",
                  background: "var(--bg-card)",
                  padding: 0,
                  boxShadow: sceneId === sc.id ? "0 0 0 3px rgba(236,72,153,0.2)" : undefined,
              ...
              >
                ...
                {sceneId === sc.id && (
                  <div style={{ position: "absolute", top: 8, right: 8, width: 22, height: 22, borderRadius: "50%", background: "#ec4899", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Check size={12} color="#fff" />
                  </div>
                )}
                {sc.hasColorPicker && (
                  <div style={{ position: "absolute", top: 8, left: 8, background: "rgba(0,0,0,0.6)", borderRadius: 6, padding: "2px 6px", fontSize: 10, color: "#fff", fontWeight: 600 }}>12 COLORS</div>
                )}
          ...
          {/* Color swatch sub-picker */}
          {scene.hasColorPicker && (
            <div style={{ marginBottom: 24 }}>
              <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 10, color: "var(--text-muted)" }}>Choose backdrop color</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {BACKDROP_COLORS.map(c => (
                  <button
                    key={c.id}
                    title={c.name}
                    onClick={() => setBackdropColorId(c.id)}
                    style={{
                      width: 36,
                      height: 36,
                      borderRadius: 8,
                      background: c.swatch,
                      border: `3px solid ${backdropColorId === c.id ? "#fff" : "transparent"}`,
                      boxShadow: backdropColorId === c.id ? `0 0 0 2px #ec4899` : `0 0 0 1px var(--border)`,
                      cursor: "pointer",
                      padding: 0,
              ...
              <div style={{ marginTop: 8, fontSize: 12, color: "var(--text-muted)" }}>
                Selected: <strong style={{ color: backdropColor.swatch }}>{backdropColor.name}</strong>
              </div>
            </div>
          )}
                    ...
                    {g.status === "completed" && g.result_image_url ? (
                      <>
                        ...
                        <div style={{ padding: "8px 10px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                          ...
                          <button onClick={() => download(g.result_image_url!, `artist-shoot-${g.id}.jpg`)} style={{ border: "none", background: "transparent", cursor: "pointer", color: "var(--text-muted)" }}><Download size={14} /></button>
                        </div>
                      </>
                    ) : (
                      <div style={{ aspectRatio: "1", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8 }}>
                        {(g.status === "pending" || g.status === "processing") && <Loader2 size={20} style={{ animation: "spin 1s linear infinite", color: "#ec4899" }} />}
                        <StatusDot status={g.status} />
                      </div>
                    )}
                  </div>
        ...
        <div style={{ position: "sticky", top: 86 }}>
          <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: 20 }}>
            <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 4 }}>{scene.name}</div>
            ...
            <button
              ...
              style={{
                ...
                aspectRatio: "1",
                borderRadius: 12,
                border: `2px dashed ${refFile ? "#ec4899" : "var(--border)"}`,
                background: "var(--bg)",
                cursor: "pointer",
                overflow: "hidden",
                padding: 0,
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 10,
                border: "none",
                background: !refFile || generating ? "var(--border)" : "linear-gradient(135deg, #ec4899, #f97316)",
            ...
            {!loadingGallery && (
              <button onClick={() => void fetchGallery()} style={{ width: "100%", marginTop: 10, padding: "9px 0", borderRadius: 8, border: "1px solid var(--border)", background: "transparent", color: "var(--text-muted)", cursor: "pointer", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
