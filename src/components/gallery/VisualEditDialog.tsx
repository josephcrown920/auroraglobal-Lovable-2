...
export function VisualEditDialog({
  ...
  const mut = useMutation({
    ...
    onSuccess: (res) => {
      ...
      toast.success("Edit complete · 10 Aura");
    ...
    <Dialog open={open} onOpenChange={(v) => { onOpenChange(v); if (!v) { setResultUrl(null); setPrompt(""); } }}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>Tweak this shot with a prompt or a one-click preset. Each edit costs 10 Aura and saves as a new gallery entry.</DialogDescription>
        </DialogHeader>
