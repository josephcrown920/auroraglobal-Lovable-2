...
// Compatibility average; photo charging uses the exact 85-Aura package and
// deterministic integer per-result allocations for accurate partial refunds.
...
export const spinThirty = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const spinCost = mode === "video" ? SPIN_COUNT * costPerPiece : SPIN_PACKAGE_COST;
        ...
        try {
          ...
          if (((won ?? []) as { id: string }[]).length > 0) {
            await supabaseAdmin.from("generations").insert({
              ...
              credits_cost: mode === "video" ? costPerPiece : spinPieceCost(p.idx),
            } as never);
          }
        } catch (e) {
          ...
          if (((lost ?? []) as { id: string }[]).length > 0) {
            ...
            if (!admin) {
              await supabaseAdmin.rpc("grant_credits", {
                _user: job.user_id,
                _amount: mode === "video" ? costPerPiece : spinPieceCost(p.idx),
                _reason: "refund_failed_generation",
                _ref: p.id,
              });
            }
