...
export const chatWithAuroraAgent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: memRow }, { data: recent, error: histErr }] = await Promise.all([
      (context.supabase as any).from("agent_user_memory").select("memory").eq("user_id", context.userId).maybeSingle(),
    ...
    const transcript = ((recent ?? []) as Array<{ role: string; content: string }>)
      .reverse()
      .map((m) => ({
    ...
    if (turn.memoryUpdate && turn.memoryUpdate.trim()) {
      const { error: memErr } = await (context.supabase as any).from("agent_user_memory").upsert({
        user_id: context.userId,
