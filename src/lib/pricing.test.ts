...
describe("computeCost — lyric_video (flat, non-length-scaled)", () => {
  ...
  it("stacks additively with other features", () => {
    expect(computeCost({ features: ["lyric_video", "image"] }).total).toBe(15);
  });
});
...
describe("computeCost — rounding & stacking", () => {
  ...
  it("sums each active feature when stacked", () => {
    // image + audio at baseline = 10 + 2 = 12
    expect(computeCost({ features: ["image", "audio"] }).total).toBe(12);
  });
  ...
  it("budget-tier video + motion + lip-sync stacks correctly at 1080p/10s", () => {
    ...
    // image(10) + video(40) + motion(30×2×2=120) + lipsync(3×2=6) = 176
    expect(video.total + lip.total).toBe(176);
  });
...
describe("detectFeatures — conservative, deterministic", () => {
  ...
  it("override is additive only — the primary kind is always billed (no undercharge)", () => {
    ...
    // charged for both the requested image and the video they actually run.
    ...
    expect(total).toBe(20); // image(10) + video(10) — strictly more than video-only's 10
  });
});
