...
CREATE POLICY "own webhooks select" ON public.user_webhooks FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "own webhooks insert" ON public.user_webhooks FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "own webhooks update" ON public.user_webhooks FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "own webhooks delete" ON public.user_webhooks FOR DELETE USING (auth.uid() = user_id);
